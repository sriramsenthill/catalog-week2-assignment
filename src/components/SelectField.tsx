...
const SelectField = ({ label, name, value, options, onChange, error }: SelectFieldProps) => (
    <div className="mb-4 flex flex-col w-full gap-y-2">
        ...
        <select
            ...
            value={value}
            onChange={onChange}
            className={` text-custom-gray shadow-slate-300 rounded-xl  transition-colors duration-200 hover:bg-gray-50  p-4 w-full  focus:outline-none ${error ? 'border-red-500' : ''}`}
        >
